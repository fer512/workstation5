...
import { EntityArrayResponseType, BranchService } from '../service/branch.service';
...
export class BranchComponent implements OnInit {
  ...
  trackId = (_index: number, item: IBranch): number => this.branchService.getBranchIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.branches = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IBranch[] | null): IBranch[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.branchService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
